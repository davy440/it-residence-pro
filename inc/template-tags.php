 ...
 */
 ...
 if ( ! function_exists( 'itre_posted_on' ) ) :
 	...
 	function itre_posted_on() {
 		...
 		echo '<span class="posted-on" ' . $d_schema . '>' . $posted_on . '</span>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
 ...
 if ( ! function_exists( 'itre_posted_by' ) ) :
 	...
 	function itre_posted_by() {
 		...
 		echo '<span class="byline" ' . $a_schema .  '><i class="fa fa-user-circle-o" aria-hidden="true"></i>
...
function itre_get_property_price( $data ) {
	$currency = itre_get_currency();
	$format = get_theme_mod('itre_currency_format', 'symbol');
    ...
    $price_string = sprintf('<span class="prop-price"><span>%s</span></span>', $price->formatCurrency( $data, $currency['code'] ) );
