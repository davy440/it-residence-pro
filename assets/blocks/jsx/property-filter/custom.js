...
const filterProperties = () => {
    const filterDivs = document.querySelectorAll('.itre-property-filter');

    if (filterDivs.length === 0) {
        return;
    }
    const { filterIDs } = window;

    filterIDs.forEach(id => {
        const filterBlock = document.querySelector(`#${id}`);
        const form = filterBlock.querySelector('form');
        const propertyContainer = filterBlock.querySelector('.itre-property-listing');
        const nghbrs = propertyContainer.previousElementSibling;
        ;
        const { ajaxurl, action_filter, nonce_filter, nghbr_action, nghbr_nonce } = filter;
    
        const fetchProperties = async (body) => {
        ...
        const updateProperties = async (requestBody) => {
            ...
            const filteredProperties = await fetchProperties(requestBody);
        ...
        const nghbrRequest = async (location) => {
            ...
            return await fetchProperties(body);
        }
